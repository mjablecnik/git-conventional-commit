...
- (Optional) OpenAI API key for AI commit message generation (see below)

## Features

- Generate [Conventional Commits](https://www.conventionalcommits.org/) easily from the command line
- AI-powered commit message generation (ChatGPT, optional)
...
- If the AI generation fails, you will be prompted to try again or enter a message manually.
- This feature is optional; if no API key is present, only manual commit messages are supported.

#### Example (AI-generated message):

```sh
git-conventional-commit -t feat -s core
# (no -m provided, message will be generated by AI)
```
